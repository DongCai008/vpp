...
dp_add_fwd_entry_from_mt (u32 si, u32 di)
{
  ...
  vlib_rpc_call_main_thread (dp_add_fwd_entry_thread_fn, (u8 *) &a, sizeof (a));
  return 0;
}
...
lisp_key_type_to_crypto_alg (lisp_key_type_t key_id)
{
  switch (key_id)
    ...
    case HMAC_SHA_256_128:
    return VNET_CRYPTO_ALG_SHA2_256;
    default:
...
{
  vlib_rpc_call_main_thread (process_map_reply, (u8 *) a, sizeof (*a));
}

static void
queue_map_notify_for_processing (map_records_arg_t * a)
{
  vlib_rpc_call_main_thread (process_map_notify, (u8 *) a, sizeof (a[0]));
}
		   ...
		   u8 smr_invoked, u8 is_resend)
{
  ...
  vlib_rpc_call_main_thread (send_map_request_thread_fn, (u8 *) &a, sizeof (a));
  return 0;
}
