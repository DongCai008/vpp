...
ip4_rewrite_make_shared_views_writable (vlib_main_t *vm, vlib_node_runtime_t *error_node, u32 *from,
...
always_inline uword
ip4_rewrite_inline (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame,
		    int do_counters, int is_midchain, int is_mcast)
{
  ...
  u16 nexts[VLIB_FRAME_SIZE], *next;
  u32 n_left_from, n_failed, n_packets, n_writable;
  vlib_node_runtime_t *error_node = vlib_node_get_runtime (vm, ip4_input_node.index);

  n_packets = frame->n_vectors;
  n_writable = ip4_rewrite_make_shared_views_writable (vm, error_node, from, n_packets, failed);
  ...
  vlib_buffer_enqueue_to_next (vm, node, from, nexts, n_writable);
  if (n_failed)
    {
      clib_memset_u16 (nexts, IP4_REWRITE_NEXT_DROP, n_failed);
      vlib_buffer_enqueue_to_next (vm, node, failed, nexts, n_failed);
    }
  return n_packets;
}
