		   ...
		   int is_ip6)
{
  ...
  while (n_left > 0)
      ...
      if (PREDICT_TRUE (ort->cached.integ_icv_size && !ort->cached.is_aead))
	{
	  ...
	  vnet_crypto_ctx_t *ctx = ort->ctx;
	  vnet_crypto_op_init (ctx, op);
	  op->type = VNET_CRYPTO_OP_TYPE_HMAC;
	  op->auth_src = vlib_buffer_get_current (b[0]);
	  op->auth_src_len = b[0]->current_length;
	  op->auth = vlib_buffer_get_current (b[0]) + ip_hdr_size + sizeof (ah_header_t);
	  clib_memset (op->auth, 0, icv_size);
	  op->auth_len = icv_size;
	  op->user_data = b - bufs;
	  if (ort->cached.use_esn)
	    {
	      *(u32u *) (op->auth_src + b[0]->current_length) =
		clib_host_to_net_u32 (ort->seq64 >> 32);
	      op->auth_src_len += sizeof (u32);
	    }
	}
