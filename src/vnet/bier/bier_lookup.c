...
bier_lookup_make_writable (vlib_main_t *vm, vlib_node_runtime_t *node, u32 *bi0, vlib_buffer_t **b0,
			   u32 bti0, u32 next_index, u32 **to_next, u32 *n_left_to_next)
{
...
static uword
bier_lookup (vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (PREDICT_FALSE (bier_lookup_make_writable (vm, node, &bi0, &b0, bti0, next_index,
	  ...
	  if (PREDICT_TRUE (0 != n_clones))
	    {
	      ...
	      num_cloned = vlib_buffer_clone (vm, bi0, blm->blm_clones[thread_index], n_clones,
  ...
  vlib_node_increment_counter (vm, bier_lookup_node.index, BIER_LOOKUP_ERROR_NONE,
