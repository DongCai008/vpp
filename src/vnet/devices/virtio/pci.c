...
virtio_pci_ctrl_mac_addr_set (vlib_main_t *vm, virtio_if_t *vif, const u8 *address)
{
  virtio_ctrl_msg_t msg;
  virtio_net_ctrl_ack_t status = VIRTIO_NET_ERR;
  u64 mac_ctrl_features =
    VIRTIO_FEATURE (VIRTIO_NET_F_CTRL_RX) | VIRTIO_FEATURE (VIRTIO_NET_F_CTRL_MAC_ADDR);

  /* VIRTIO_NET_CTRL_MAC class commands require both VIRTIO_NET_F_CTRL_RX and
     VIRTIO_NET_F_CTRL_MAC_ADDR (Section 5.1.6.5.2.2 - Driver Requirements) */
  if ((vif->features & mac_ctrl_features) == mac_ctrl_features)
    {
      clib_memset (&msg, 0, sizeof (msg));
      msg.ctrl.class = VIRTIO_NET_CTRL_MAC;
...
virtio_pci_set_mac_addr (vlib_main_t *vm, virtio_if_t *vif, const u8 *address)
{
  if (virtio_pci_ctrl_mac_addr_set (vm, vif, address) != 0)
    {
      virtio_log_warning (vif, "falling back to config space to set MAC address");
