...
vnet_hw_interface_change_link_speed (vnet_main_t *vnm, u32 hw_if_index, u32 link_speed_kbps)
{
  vnet_hw_interface_t *hw = vnet_get_hw_interface (vnm, hw_if_index);
  ...
  if (hw->supported_link_speeds && !(hw->supported_link_speeds & speed))
    return clib_error_return (0, "speed %u Kbps not supported by interface", link_speed_kbps);
